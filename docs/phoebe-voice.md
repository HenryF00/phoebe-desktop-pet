...
3. 第一次运行时安全展开到本机应用数据目录，执行 `conda-unpack` 完成路径迁移；完成标记写入后，后续启动直接复用。展开与加载分两段：展开阶段（首次数 GB）会发出「首次准备菲比声音…」进度，加载阶段单独设 `SERVER_LOAD_TIMEOUT`（180 秒）超时。
4. 使用固定配置启动 `api_v2.py`，日志和缓存写入应用数据／缓存目录，不修改 `.app`。启动失败或超时会在提示里附上 `logs/gpt-sovits.log` 路径。
5. 运行包就绪后会清理旧的 `runtime_id` 展开目录与残留的 `.installing-*`，避免多份数 GB 目录累积。
6. 应用正常退出时只终止自己持有的子进程；复用的外部 9880 服务不会被关闭。
...
- 生日当天首次对话：直接播放内置的「017_生日祝福」预录原声，不经 TTS 合成（不改写、不复用参考音频）。
