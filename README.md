...
点击人物打字或长按麦克风说话，双击人物进入全部设置，双击对话气泡查看聊天记录。字幕流式追加，声音在本机合成。当前版本 **1.12.0**。

这是非官方角色桌宠实验项目。角色素材与用户提供的语音样本保存在 `assets/`；本仓库不包含语音模型权重、Python 环境或聊天记录。

## 当前完成情况

- **1.12.0 连续课堂**：进入小黑板后，洛琪希停靠右下方，普通输入收起；在黑板下方打字或长按麦克风继续提问，保留上一轮问题，沿用本课资料与问答。在同一块黑板更新讲解，最后独立播放结束语；关闭黑板恢复人物位置与普通输入。内置小赖手写字体，无需系统安装；修复教鞭帧导出时的色彩空间提亮。见 [连续课堂说明](docs/lecture-conversation-1.12.md)。
- **1.11.0 黑板与行情**：木框深绿板面、平滑粉笔红圈、单一右上设置按钮；美股历史日线提供大幅 K 线／面积走势及成交量，日期与来源随图展示。见 [黑板与行情说明](docs/blackboard-refresh-1.11.md)。
...
- **1.10.1 授课与快捷入口**：输入 `@` 搜索和补全项目，输入 `/` 选择委托／小黑板／聊天。候选列表与无提示文字的聊天框统一为半透明圆角样式。授课字幕独立附在黑板下方，同步中／英／日字幕；重点只用红色粉笔圈。已有校准形象编排抬手讲解、转向板书和点头强调，跟随实际语音播放。
- **1.9.0 小黑板讲解**：委托结果显示结论、要点、可核对的数值图表与来源；洛琪希用重点讲解代替照读全文。右上角蓝色转圈表示任务进行中，蓝点表示未读结果。双击人物 → 任务与小黑板，可选择自动通报或点击查看。见 [小黑板使用说明](docs/teaching-board.md)。
- **1.8.0 Codex 委托**：支持文字／截图粘贴，右键选择 Codex 保存的项目，或用“委托：……”发起联网分析；后台执行后按设置展示小黑板和重点讲解。任务列表支持授权、回答问题、取消和继续。见 [任务委托使用说明](docs/delegated-tasks.md)。
- **1.7.0 聊天动作与角色记忆**：等待模型时托腮思考，录音时专注倾听，回复表情跟随实际音频播放。接入独立角色卡；双击人物 →“记住的事…”可管理长期偏好，也支持“请记住：……”与“忘记：完全匹配的内容”。详见 [聊天动作与记忆](docs/companion-behavior.md)。
